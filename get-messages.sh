...
# the next line containing "---- End of full list of messages ----" or the line containing "##[endgroup]"
awk '/---- Full list of messages ----/{flag=1;next}/---- End of full list of messages ----|##\[endgroup\]/{flag=0}flag' "$log_file" | \
...
# Select only the messages that contain "level" of "Error"
# jq -r '.[] | select(.level == "Error")' | \
# Extract the "code" and "message" field from each message and print on a single line
# jq -r '"\(.code) \(.message)"'
# cat > messages.json
jq -r '.[] | select(.code == "RemovedProperty")'
